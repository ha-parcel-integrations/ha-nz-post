"""Tests for NZ Post sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.nz_post.const import ParcelStatus
from custom_components.nz_post.sensor import (
    NZPostDeliveredParcelsSensor,
    NZPostIncomingParcelsSensor,
    NZPostLastUpdateSensor,
    NZPostNextDeliverySensor,
    NZPostParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = NZPostIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = NZPostParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = NZPostParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = NZPostNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = NZPostNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = NZPostDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = NZPostLastUpdateSensor(coordinator, _entry())
