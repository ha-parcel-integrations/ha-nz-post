"""Tests for the NZ Post coordinator: fetching, caching and events.
...
from custom_components.nz_post.api import NZPostApiError
...
from custom_components.nz_post.coordinator import NZPostCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["tracking_events"][0]["status"] = "Processed at depot"
    sample["tracking_events"][0]["edifact_code"] = "8"
    return sample
...
async def test_update_merges_multiple_parcels(hass):
    ...
    coordinator = NZPostCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = NZPostApiError("HTTP 500")
    await coordinator._async_update_data()  # error -> cached raw reused
    assert len(coordinator.delivered) == 1


async def test_cache_never_retains_carrier_pii(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: DELIVERED_CODE}])
    entry.add_to_hass(hass)
    sample = delivered_sample()
    sample["tracking_events"][0].update(
        {"signed_by": "Jane Doe", "depot_name": "Private depot", "run_name": "Run 4"}
    )
    client = AsyncMock()
    ...
    assert "Jane Doe" not in str(cached)
    assert "Private depot" not in str(cached)
    assert "Run 4" not in str(cached)
    assert set(cached) == {"tracking_reference", "tracking_events"}
...
async def test_fetch_failure_log_includes_error_detail_but_not_tracking_code(
    hass, caplog
):
    from homeassistant.helpers.update_coordinator import UpdateFailed

    secret_code = "NZPOST-PRIVATE-123"
    entry = _entry_with([{CONF_TRACKING_CODE: secret_code}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = NZPostApiError("HTTP 500")
    coordinator = NZPostCoordinator(hass, client, entry)
    ...
    # The tracking code is never interpolated into the log line, but the
    # exception's own detail is — unlike the class name alone, that's enough
    # to tell a 429 from a malformed body when triaging a user's issue.
    assert secret_code not in caplog.text
    assert "NZPostApiError" in caplog.text
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = NZPostCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"tracking_reference": "GONE"}
...
async def test_does_not_fire_delivery_time_event_without_an_eta(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = NZPostCoordinator(hass, client, entry)
