"""Sensor platform for the NZ Post parcel tracker integration."""
...
from . import NZPostConfigEntry
from .const import DOMAIN
from .coordinator import NZPostCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: NZPostConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up NZ Post sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        NZPostIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            NZPostParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(NZPostNextDeliverySensor(coordinator, entry))
    entities.append(NZPostDeliveredParcelsSensor(coordinator, entry))
    entities.append(NZPostLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class NZPostIncomingParcelsSensor(
    CoordinatorEntity[NZPostCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                NZPostParcelSensor(self.coordinator, self._entry, barcode)
...
class NZPostParcelSensor(CoordinatorEntity[NZPostCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked NZ Post parcel."""
    ...
    def __init__(
        self, coordinator: NZPostCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class NZPostNextDeliverySensor(
    CoordinatorEntity[NZPostCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: NZPostCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class NZPostDeliveredParcelsSensor(
    CoordinatorEntity[NZPostCoordinator], SensorEntity
):
    """Recently delivered tracked NZ Post parcels."""
...
class NZPostLastUpdateSensor(
    CoordinatorEntity[NZPostCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when NZ Post was last polled successfully."""
