"""Coordinator for the NZ Post parcel tracker integration.
...
from .api import NZPostApiClient, NZPostApiError
from .const import (
...
class NZPostCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (NZPostApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, NZPostApiError)
                ...
                _LOGGER.warning(
                    "NZ Post fetch failed (%s): %s; retaining safe cached data "
                    "when available",
            ...
            if result is None:
                ...
                raws.append(self._raw_cache.get(code) or {"tracking_reference": code})
                continue

            # The response's own tracking number can be missing on edge
            # payloads; fall back to the code we asked for so the sensor keeps
            # its key.
            result.setdefault("tracking_reference", code)
            safe_result = sanitize_raw_parcel(result)
            self._raw_cache[code] = safe_result
            raws.append(safe_result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "NZ Post rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("NZ Post unreachable for all tracked parcels")
