"""Config flow for the NZ Post parcel tracker integration."""
...
def normalize_tracking_code(value: str) -> str:
    """Return an NZ Post tracking code without imposing a format mask."""
    return str(value or "").strip().upper()


def valid_tracking_code(value: str) -> bool:
    """Whether ``value`` looks like a NZ Post tracking code."""
    # NZ Post supports numeric, UPU and other alphanumeric references. Let the
    # tracker decide whether a non-empty reference exists rather than rejecting
    # a valid format locally.
    return bool(value)
...
class NZPostConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the NZ Post integration."""
    ...
    ) -> NZPostOptionsFlowHandler:
        """Return the options flow handler."""
        return NZPostOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the NZ Post hub — single instance, no input needed.
        ...
        the ``nz_post.track_parcel`` service or a dashboard button.
...
class NZPostOptionsFlowHandler(OptionsFlow):
