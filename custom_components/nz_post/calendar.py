"""Calendar platform for the NZ Post parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the NZ Post deliveries calendar from a config entry."""
    async_add_entities([NZPostDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class NZPostDeliveriesCalendar(CoordinatorEntity[NZPostCoordinator], CalendarEntity):
    """A read-only calendar of expected NZ Post deliveries.
    ...
    def __init__(self, coordinator: NZPostCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "NZ Post parcel")
            description_parts = [
