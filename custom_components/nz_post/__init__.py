"""NZ Post parcel tracker custom component for Home Assistant."""
...
from .api import NZPostApiClient
...
@dataclass
class NZPostData:
    """Runtime data attached to the NZ Post config entry."""

    client: NZPostApiClient
    coordinator: NZPostCoordinator


type NZPostConfigEntry = ConfigEntry[NZPostData]


async def async_setup_entry(hass: HomeAssistant, entry: NZPostConfigEntry) -> bool:
    """Set up NZ Post from a config entry."""
    # No auth: NZ Post tracking is public, so the HA-managed session is fine.
    client = NZPostApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = NZPostData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: NZPostConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: NZPostConfigEntry) -> bool:
    """Unload the NZ Post config entry."""
