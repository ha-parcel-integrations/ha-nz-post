# Automatic parcel tracking from e-mail (IMAP → NZ Post)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the NZ Post tracking code, and register it with `nz_post.track_parcel` — fully automatic, no extra custom component required.

NZ Post is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `nz_post.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ nz_post.track_parcel
                                     │
...
- This integration, with the `nz_post.track_parcel` action available (field `tracking_code`).
...
NZ Post has no confirmed safe e-mail prefix. Do not enable the example's placeholder pattern: replace it with a specific, personally verified pattern from your own notification e-mails. The integration itself accepts broad tracking references; an overly broad e-mail matcher would also pick up order numbers.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@nz_postshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `nz_post.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
